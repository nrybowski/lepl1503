...
void enqueue_mem_check(int delta, int stats, int ret){
  ...
  if(stats > 1){
	flag_enqueue = 0;
	push_info_msg(_("Why do you use malloc more then once"));
  }
...
void test_dequeue_one() {
    ...
    if(delta){
	flag_dequeue = 0;
	push_info_msg(_("You shouldn't allocate memory for dequeue"));
    }
	...
	flag_dequeue = 0;
	push_info_msg(_("You should use free once !"));
    }
    CU_ASSERT_EQUAL(ret,1);
    if(ret != 1){
	flag_dequeue = 0;
    ...
    if(q.size){
	flag_dequeue = 0;
	push_info_msg(_("You have to change the size of the queue after the dequeue"));
    }
}
