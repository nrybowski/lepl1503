...
void init_mutex(){
    ...
    if(err){
        push_info_msg(_("An internal error occured when initializing mutex in CTester.c, please contact your tutor or an administrator"));
    }
}

void clean_mutex(){
    int err = pthread_mutex_destroy(&mut);
    if(err){
        char* msg = _("An internal error occured when destroying mutex in CTester.c, please contact your tutor or an administrator : %d");
        char errmsg[strlen(msg)+30];
        sprintf(errmsg,msg,err);
